   ...
   \\ \\/  \\/ / _ \\ |/ __/ _ \\| '_ ` _ \\ / _ \\ | |
    \\  /\\  /  __/ | (_| (_) | | | | | |  __/ |_|
     \\/  \\/ \\___|_|\\___\\___/|_| |_| |_|\\___| (_)
        ...
        self.get_random_word(self.list_of_words,
                             self.read_word_list_length(self.DICTIONARY))
        self.FILE.close()
        self.create_hidden_word(self.mk_string(self.word_to_guess, ""))
        self.play_game()
        while not self.game_complete and self.number_of_guesses != 0:
            os.system('clear')
            self.play_game()
    ...
    def create_hidden_word(self, wordToHide):
        for _ in range(0, len(wordToHide)):
